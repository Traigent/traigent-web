...
// Reference proof for the competitive claims. FACTS (capabilities · pricing ·
// funding) are cited; comparative CALLS (red/yellow "who leads", "no equivalent",
// the moats) are Traigent assessment, noted as such. src = [[label, url], …];
// note = provenance when there's no clean public URL.
const REFERENCES = [
  { name: "Arize AI / Phoenix", what: "Tracing · eval · drift (#5,#8,#9,#13); Phoenix OSS. Free / $50 / Enterprise ~$60K/yr. $70M Series C.", src: [["pricing", "https://arize.com/pricing"], ["Phoenix (GitHub)", "https://github.com/Arize-ai/phoenix"], ["$70M Series C", "https://www.prnewswire.com/news-releases/arize-ai-secures-70m-series-c-to-fix-ais-biggest-problem-making-llms-and-ai-agents-work-in-the-real-world-302381601.html"]] },
  { name: "Braintrust", what: "Eval frameworks · experiments · prompt iteration (#1,#3,#6,#10,#12). Pro $249/mo · Enterprise $100K+/yr.", src: [["pricing", "https://www.braintrust.dev/pricing"], ["profile", "https://www.cbinsights.com/company/braintrust-data"]] },
  { name: "Galileo", what: "Eval + Luna SLM evaluators ($0.02/1M tok) · guardrails. Fortune-50 focus. $45M Series B.", src: [["pricing", "https://galileo.ai/pricing"], ["$45M Series B", "https://www.prnewswire.com/news-releases/galileo-raises-45m-series-b-funding-to-bring-evaluation-intelligence-to-generative-ai-teams-everywhere-302276383.html"]] },
  { name: "Langfuse", what: "Mature tracing / observability (OSS, 26M+ installs/mo). Free / $29 / $199 / $2,499. Acquired by ClickHouse.", src: [["pricing", "https://langfuse.com/pricing"], ["GitHub", "https://github.com/langfuse/langfuse"], ["ClickHouse acq.", "https://clickhouse.com/blog/clickhouse-acquires-langfuse-open-source-llm-observability"]] },
  { name: "Portkey", what: "AI gateway — routing · caching · spend caps · guardrails (#2,#7,#8,#10). Free / $49 / Enterprise.", src: [["AI gateway", "https://portkey.ai/features/ai-gateway"], ["GitHub", "https://github.com/Portkey-AI/gateway"]] },
  { name: "LangSmith (LangChain)", what: "Tracing · eval · prompt canvas · CI (#1,#5,#6,#10,#12). Free / $39 per-seat / Enterprise.", src: [["pricing", "https://www.langchain.com/pricing"], ["observability", "https://www.langchain.com/langsmith/observability"]] },
  { name: "Helicone", what: "Observability + AI gateway. Free / $20 per-seat / Team ~$200 / Enterprise.", src: [["pricing", "https://www.helicone.ai/pricing"], ["GitHub", "https://github.com/Helicone/helicone"]] },
  { name: "DeepEval / Confident AI", what: "Eval framework (OSS) · GEPA prompt-opt · CI · RAG metrics (#1,#3,#6,#12,#16). $20–80/seat. $2.2M seed (YC).", src: [["pricing", "https://www.confident-ai.com/pricing"], ["GitHub", "https://github.com/confident-ai/deepeval"], ["YC", "https://www.ycombinator.com/companies/confident-ai"]] },
  { name: "Arato", what: "Agent testing / adversarial simulation (#1). $19,200/yr Advanced · $28,800/yr Enterprise. $10M seed (TLV Partners).", src: [], note: "pricing per Arato AWS Marketplace listing; funding per internal comp research" },
  { name: "Plurai (IntellAgent)", what: "Adversarial multi-agent simulation (OSS IntellAgent) + research paper. Early-stage.", src: [["site", "https://www.plurai.ai/"], ["IntellAgent (GitHub)", "https://github.com/plurai-ai/intellagent"], ["paper (arXiv)", "https://arxiv.org/abs/2501.11067"]] },
  { name: "DSPy (Stanford)", what: "OSS prompt / program optimizer — the partial-improve challenger to moat ① (footnote).", src: [["GitHub", "https://github.com/stanfordnlp/dspy"], ["paper", "https://arxiv.org/abs/2310.03714"]] },
  { name: "W&B Weave", what: "Tracing / eval (#5).", src: [["docs", "https://docs.wandb.ai/weave"], ["pricing", "https://wandb.ai/site/pricing"]] },
  { name: "Avon AI", what: "Runtime guardrails + governance for regulated (banks · insurance) (#7,#13,#15). Pre-seed.", src: [], note: "internal comp research (workbook); pricing not public" },
  { name: "Guardrails AI", what: "OSS output-validation framework — runtime guardrail (#1,#7).", src: [["GitHub", "https://github.com/guardrails-ai/guardrails"]] },
  { name: "NeMo Guardrails (NVIDIA)", what: "OSS rails toolkit — in-process guardrail (#7).", src: [["GitHub", "https://github.com/NVIDIA/NeMo-Guardrails"]] },
  { name: "Llama Guard (Meta)", what: "Safety classifier model — external guardrail (#1,#7).", src: [["Meta / Hugging Face", "https://huggingface.co/meta-llama"]] },
  { name: "Ragas", what: "OSS RAG evaluation metrics (#12).", src: [["GitHub", "https://github.com/explodinggradients/ragas"]] },
  { name: "Martian / RouteLLM", what: "Per-request model routing (#2,#8,#10).", src: [["RouteLLM (LMSYS)", "https://github.com/lm-sys/RouteLLM"]] },
];
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-slate-200 px-4 py-6">
      ...
      <div className="max-w-[1560px] mx-auto">
        ...
        {/* Footnote */}
        <p className="bg-slate-900/50 text-[11px] text-slate-500 px-5 py-3 leading-relaxed">
        ...
        <div className="rounded-b-lg bg-slate-950/60 border-t border-slate-800 px-5 py-4">
          <div className="text-[12px] font-bold text-white mb-1">Sources &amp; confidence</div>
          <p className="text-[11px] text-slate-400 leading-relaxed mb-3">
            Two kinds of claim on this board. <span className="text-slate-200 font-semibold">Sourced facts</span> — what each tool
            does, its pricing, and funding — are cited below. The <span className="text-slate-200 font-semibold">comparative calls</span>
            {" "}(<span className="text-red-400">red</span> = they lead · <span className="text-yellow-400">yellow</span> = they lead
            this cell but a moat wins the pain · &ldquo;no equivalent competitor offering&rdquo; · the four moats) are
            {" "}<span className="text-slate-200 font-semibold">Traigent's assessment</span>, reasoned from these facts + hands-on
            evaluation — not the competitors' own claims. &ldquo;In progress&rdquo; marks a Traigent roadmap item, not shipped.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
            {REFERENCES.map((r) => (
              <div key={r.name} className="text-[11px] leading-snug border-b border-slate-800/70 pb-2">
                <span className="text-slate-200 font-semibold">{r.name}</span>
                <span className="text-slate-400"> — {r.what}</span>
                {r.src.length > 0 && <span className="text-slate-600"> · </span>}
                {r.src.map(([t, u], i) => (
                  <span key={u}>
                    {i > 0 && <span className="text-slate-600"> · </span>}
                    <a href={u} target="_blank" rel="noopener noreferrer" className="text-[#4D8EF8] hover:underline">{t}</a>
                  </span>
                ))}
